 ...
 */
static void
ETH_Delay(__IO uint32_t nCount)
{
 ...
 */
uint32_t
ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
    ...
    if (ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
    {
        ...
        if (timeout == PHY_READ_TO)
        {
            ...
            if (timeout == PHY_READ_TO)
            {
                logmsg("ETH auto-negotiation failed\n");
                return ETH_ERROR;
            }
        }

        logmsg("ETH auto-negotiation be completed\n");
        /* Reset Timeout counter */
        timeout = 0;
        ...
        //logmsg("PHY_SR :%x \n ", RegValue);
        if ((RegValue & DM9161_AUTONEG_COMPLETE) == (uint32_t) RESET){
            logmsg("ETH auto-negotiation failed \n");
            return ETH_ERROR;
        }
        if ((RegValue & DM9161_100FDX) != (uint32_t) RESET)
        {

            ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
            ETH_InitStruct->ETH_Speed = ETH_Speed_100M;
            logmsg("ETH Speed:100M Mode:FullDuplex\n");

        }
        else if ((RegValue & DM9161_10FDX) != (uint32_t) RESET)
        {

            ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
            ETH_InitStruct->ETH_Speed = ETH_Speed_10M;
            logmsg("ETH Speed:10M Mode:FullDuplex\n");
        }
        else if ((RegValue & DM9161_100HDX) != (uint32_t) RESET)
        {

            ETH_InitStruct->ETH_Mode = ETH_Mode_HalfDuplex;
            ETH_InitStruct->ETH_Speed = ETH_Speed_100M;
            logmsg("ETH Speed:100M Mode:HalfDuplex\n");

        }
        else if ((RegValue & DM9161_10HDX) != (uint32_t) RESET)
        {

            ETH_InitStruct->ETH_Mode = ETH_Mode_HalfDuplex;
            ETH_InitStruct->ETH_Speed = ETH_Speed_10M;
            logmsg("ETH Speed:10M Mode:HalfDuplex\n");
        }
    ...
    tmpreg |= (uint32_t) (ETH_InitStruct->ETH_Watchdog
            | ETH_InitStruct->ETH_Jabber | ETH_InitStruct->ETH_InterFrameGap
            | ETH_InitStruct->ETH_CarrierSense | ETH_InitStruct->ETH_Speed
            | ETH_InitStruct->ETH_ReceiveOwn | ETH_InitStruct->ETH_LoopbackMode
            | ETH_InitStruct->ETH_Mode | ETH_InitStruct->ETH_ChecksumOffload
    ...
    /* Write to ETHERNET MACFFR */ETH->MACFFR =
            (uint32_t) (ETH_InitStruct->ETH_ReceiveAll
    ...
    /* Set the VL bit according to ETH_VLANTagIdentifier value */ETH->MACVLANTR =
            (uint32_t) (ETH_InitStruct->ETH_VLANTagComparison
    ...
    /* Set the PR and DA bits according to ETH_DMAArbitration value */ETH->DMABMR =
            (uint32_t) (ETH_InitStruct->ETH_AddressAlignedBeats
                    | ETH_InitStruct->ETH_FixedBurst
                    | ETH_InitStruct->ETH_RxDMABurstLength | /* !! if 4xPBL is selected for Tx or Rx it is applied for the other */
                    ETH_InitStruct->ETH_TxDMABurstLength
                    | (ETH_InitStruct->ETH_DescriptorSkipLength << 2)
                    | ETH_InitStruct->ETH_DMAArbitration | ETH_DMABMR_USP); /* Enable use of separate PBL for Rx and Tx */
 ...
 */
void
ETH_MACTransmissionCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_MACReceptionCmd(FunctionalState NewState)
{
 ...
 */
FlagStatus
ETH_GetFlowControlBusyStatus(void)
{
 ...
 */
void
ETH_InitiatePauseControlFrame(void)
{
 ...
 */
void
ETH_BackPressureActivationCmd(FunctionalState NewState)
{
 ...
 */
ITStatus
ETH_GetMACITStatus(uint32_t ETH_MAC_IT)
{
 ...
 */
void
ETH_MACITConfig(uint32_t ETH_MAC_IT, FunctionalState NewState)
{
 ...
 */
void
ETH_MACAddressConfig(uint32_t MacAddr, uint8_t *Addr)
{
    uint32_t tmpreg;
 ...
 */
void
ETH_GetMACAddress(uint32_t MacAddr, uint8_t *Addr)
{
    uint32_t tmpreg;
 ...
 */
void
ETH_MACAddressPerfectFilterCmd(uint32_t MacAddr, FunctionalState NewState)
{
 ...
 */
void
ETH_MACAddressFilterConfig(uint32_t MacAddr, uint32_t Filter)
{
 ...
 */
void
ETH_MACAddressMaskBytesFilterConfig(uint32_t MacAddr, uint32_t MaskByte)
{
 ...
 */
FrameTypeDef
ETH_Get_Received_Frame_interrupt(void)
{
    FrameTypeDef frame =
        { 0, 0, 0 };
 ...
 */
void
ETH_DMARxDescChainInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff,
 ...
 */
void
ETH_DMATxDescChainInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t* TxBuff,
 ...
 */
FlagStatus
ETH_GetDMATxDescFlagStatus(ETH_DMADESCTypeDef *DMATxDesc,
 ...
 */
uint32_t
ETH_GetDMATxDescCollisionCount(ETH_DMADESCTypeDef *DMATxDesc)
{
 ...
 */
void
ETH_SetDMATxDescOwnBit(ETH_DMADESCTypeDef *DMATxDesc)
{
 ...
 */
void
ETH_DMATxDescTransmitITConfig(ETH_DMADESCTypeDef *DMATxDesc,
 ...
 */
void
ETH_DMATxDescFrameSegmentConfig(ETH_DMADESCTypeDef *DMATxDesc,
 ...
 */
void
ETH_DMATxDescChecksumInsertionConfig(ETH_DMADESCTypeDef *DMATxDesc,
 ...
 */
void
ETH_DMATxDescCRCCmd(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
 ...
 */
void
ETH_DMATxDescSecondAddressChainedCmd(ETH_DMADESCTypeDef *DMATxDesc,
        FunctionalState NewState)
{
 ...
 */
void
ETH_DMATxDescShortFramePaddingCmd(ETH_DMADESCTypeDef *DMATxDesc,
        FunctionalState NewState)
{
 ...
 */
void
ETH_DMATxDescBufferSizeConfig(ETH_DMADESCTypeDef *DMATxDesc,
 ...
 */
FlagStatus
ETH_GetDMARxDescFlagStatus(ETH_DMADESCTypeDef *DMARxDesc,
 ...
 */
FlagStatus
ETH_GetDMAPTPRxDescExtendedFlagStatus(ETH_DMADESCTypeDef *DMAPTPRxDesc,
        uint32_t ETH_DMAPTPRxDescExtendedFlag)
{
    FlagStatus bitstatus = RESET;
 ...
 */
void
ETH_SetDMARxDescOwnBit(ETH_DMADESCTypeDef *DMARxDesc)
{
 ...
 */
uint32_t
ETH_GetDMARxDescFrameLength(ETH_DMADESCTypeDef *DMARxDesc)
{
    /* Return the Receive descriptor frame length */
 ...
 */
void
ETH_DMARxDescReceiveITConfig(ETH_DMADESCTypeDef *DMARxDesc,
        FunctionalState NewState)
{
 ...
 */
uint32_t
ETH_GetDMARxDescBufferSize(ETH_DMADESCTypeDef *DMARxDesc,
 ...
 */
void
ETH_EnhancedDescriptorCmd(FunctionalState NewState)
{
 ...
 */
FlagStatus
ETH_GetSoftwareResetStatus(void)
{
    FlagStatus bitstatus = RESET;
 ...
 */
FlagStatus
ETH_GetDMAFlagStatus(uint32_t ETH_DMA_FLAG)
{
    FlagStatus bitstatus = RESET;
 ...
 */
void
ETH_DMAClearFlag(uint32_t ETH_DMA_FLAG)
{
 ...
 */
void
ETH_DMAITConfig(uint32_t ETH_DMA_IT, FunctionalState NewState)
{
 ...
 */
ITStatus
ETH_GetDMAITStatus(uint32_t ETH_DMA_IT)
{
    ITStatus bitstatus = RESET;
 ...
 */
void
ETH_DMAClearITPendingBit(uint32_t ETH_DMA_IT)
{
 ...
 */
uint32_t
ETH_GetTransmitProcessState(void)
{
 ...
 */
uint32_t
ETH_GetReceiveProcessState(void)
{
 ...
 */
FlagStatus
ETH_GetFlushTransmitFIFOStatus(void)
{
    FlagStatus bitstatus = RESET;
 ...
 */
void
ETH_DMATransmissionCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_DMAReceptionCmd(FunctionalState NewState)
{
 ...
 */
FlagStatus
ETH_GetDMAOverflowStatus(uint32_t ETH_DMA_Overflow)
{
    FlagStatus bitstatus = RESET;
 ...
 */
uint32_t
ETH_GetRxOverflowMissedFrameCounter(void)
{
 ...
 */
uint32_t
ETH_GetBufferUnavailableMissedFrameCounter(void)
{
 ...
 */
uint32_t
ETH_GetCurrentTxDescStartAddress(void)
{
 ...
 */
uint32_t
ETH_GetCurrentRxDescStartAddress(void)
{
 ...
 */
uint32_t
ETH_GetCurrentTxBufferAddress(void)
{
 ...
 */
uint32_t
ETH_GetCurrentRxBufferAddress(void)
{
 ...
 */
void
ETH_ResumeDMATransmission(void)
{
    ETH->DMATPDR = 0;
}
 ...
 */
void
ETH_SetReceiveWatchdogTimer(uint8_t Value)
{
 ...
 */
uint16_t
ETH_ReadPHYRegister(uint16_t PHYAddress, uint16_t PHYReg)
{
    uint32_t tmpreg = 0;
    __IO uint32_t timeout = 0;
 ...
 */
uint32_t
ETH_WritePHYRegister(uint16_t PHYAddress, uint16_t PHYReg, uint16_t PHYValue)
{
    uint32_t tmpreg = 0;
    __IO uint32_t timeout = 0;
 ...
 */
uint32_t
ETH_PHYLoopBackCmd(uint16_t PHYAddress, FunctionalState NewState)
{
    uint16_t tmpreg = 0;
 ...
 */
void
ETH_ResetWakeUpFrameFilterRegisterPointer(void)
{
 ...
 */
void
ETH_SetWakeUpFrameFilterRegister(uint32_t *Buffer)
{
    uint32_t i = 0;
 ...
 */
void
ETH_GlobalUnicastWakeUpCmd(FunctionalState NewState)
{
 ...
 */
FlagStatus
ETH_GetPMTFlagStatus(uint32_t ETH_PMT_FLAG)
{
    FlagStatus bitstatus = RESET;
 ...
 */
void
ETH_WakeUpFrameDetectionCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_MagicPacketDetectionCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_PowerDownCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_MMCCounterFreezeCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_MMCResetOnReadCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_MMCCounterRolloverCmd(FunctionalState NewState)
{
 ...
 */
void
ETH_MMCITConfig(uint32_t ETH_MMC_IT, FunctionalState NewState)
{
 ...
 */
uint32_t
ETH_GetMMCRegister(uint32_t ETH_MMCReg)
{
...
#if 0
uint32
do_phyr(cmd_tbl_t * cmdtp, uint32 argc, uint8 *argv[])
{
    uint32 reg;
    uint32 value;
    // 参数数量=1
    ...
    value = ETH_ReadPHYRegister(0, reg);
    logmsg("phy reg:%d  value:%x\n", reg, value);
    return 0;
}

SHELL_CMD( phyr, CFG_MAXARGS, do_phyr, "v \r\t\t\t\t Read phy reg\r\n"
